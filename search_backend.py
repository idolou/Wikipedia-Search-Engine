...
def expand_query(tokens, size):
  ...
  if 4>=size>0:
    try:
      new_tokens = list(map(lambda x: x[0], model.most_similar(positive=tokens, topn=8)))
      filtered_tokens = []
  ...
  if size>=5:
    try:
      add_tokens += list(map(lambda x: x[0], model.most_similar(positive=tokens, topn=1)))
    except:
      pass
  tokens+=add_tokens
  return set(tokens)
...
def similarity(query, title):
  ...
  for token in query:
    for term in title_tokenized:
      if all(x in model.wv.vocab for x in [token,term]):
        similiarities.append(model.similarity(token, term))
