...
class MultiFileWriter:
    ...
    def __init__(self, base_dir, name, bucket_name, file_name=""):
  ...
  def read(self, locs, n_bytes, kind =""):
    ...
    for f_name, offset in locs:
      if f_name not in self._open_files:
        self._open_files[f_name] = open(Path('./postings_gcp')/(f'{kind}') / f'{f_name}', 'rb')
...
class InvertedIndex:
  ...
  def posting_lists_iter(self, kind=""):
    """ A generator that reads one posting list from disk and yields
        a (word:str, [(doc_id:int, tf:int), ...]) tuple.
    """
    with closing(MultiFileReader()) as reader:
      for w, locs in self.posting_locs.items():
        b = reader.read(locs, self.df[w] * TUPLE_SIZE, kind)
        posting_list = []
  ...
  @staticmethod
  def write_a_posting_list(b_w_pl, bucket_name, file_name = ""):
